...
namespace Microsoft.Xades
{
    ...
    public class UnsignedSignatureProperties
    {
        ...
        public bool HasChanged()
        {
            ...
            if (CounterSignatureCollection.Count > 0) retVal = true;

            if (SignatureTimeStampCollection.Count > 0) retVal = true;

            if (CompleteCertificateRefs != null && CompleteCertificateRefs.HasChanged()) retVal = true;

            if (CompleteRevocationRefs != null && CompleteRevocationRefs.HasChanged()) retVal = true;

            if (SigAndRefsTimeStampCollection.Count > 0) retVal = true;

            if (RefsOnlyTimeStampCollection.Count > 0) retVal = true;

            if (CertificateValues != null && CertificateValues.HasChanged()) retVal = true;

            if (RevocationValues != null && RevocationValues.HasChanged()) retVal = true;

            if (ArchiveTimeStampCollection.Count > 0) retVal = true;
        ...
        public void LoadXml(XmlElement xmlElement, XmlElement counterSignedXmlElement)
        {
            ...
            try
            {
                while (enumerator.MoveNext())
                {
                    ...
                    if (iterationXmlElement != null)
                    {
                        newTimeStamp = new TimeStamp("ArchiveTimeStamp", "xadesv141",
                            XadesSignedXml.XadesNamespace141Uri);
                        newTimeStamp.LoadXml(iterationXmlElement);
                        ArchiveTimeStampCollection.Add(newTimeStamp);
                    }
                }
        ...
        public XmlElement GetXml()
        {
            ...
            retVal = creationXmlDocument.CreateElement(XadesSignedXml.XmlXadesPrefix, "UnsignedSignatureProperties",
            ...
            if (CounterSignatureCollection.Count > 0)
                foreach (XadesSignedXml xadesSignedXml in CounterSignatureCollection)
                {
                    bufferXmlElement = creationXmlDocument.CreateElement(XadesSignedXml.XmlXadesPrefix,
                        "CounterSignature", XadesSignedXml.XadesNamespaceUri);
